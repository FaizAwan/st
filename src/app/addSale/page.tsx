...
const AddSale = () => {
  ...
  const submitForm = async ({
    ...
    if (Sales_date && Sales_id && customer_name && note && total_amount) {
      const db = getDatabase();
      const SalesIdRef = ref(db, 'Sales_id_counter');
      try {
        console.log("Fetching the next Sales ID...");
        const Salesnapshot = await get(SalesIdRef);
        const nextSalesId = (Salesnapshot.val() || 0) + 1; // Increment Sales ID
        console.log("Next Sales ID:", nextSalesId);
  
        const SalesData = {
          Sales_id: nextSalesId,
        ...
        // Write to the Sales table
        const SalesRef = ref(db, 'Sales/' + nextSalesId);
        await set(SalesRef, SalesData);
        // console.log("Sales data saved successfully.");
  
        // Update the Sales ID counter
        await set(SalesIdRef, nextSalesId);
  
        const SalesProductRef = ref(db, 'SalesProduct');
  
        // Push each product entry to the SalesProduct table with custom keys
        const productPromises = productTableData.map((product, index) => {
          ...
          const productKey = `${nextSalesId}_${index + 1}`;
          return set(ref(db, `SalesProduct/${productKey}`), productEntry);
        });
        ...
        console.error("Error saving Sales information:", e);
        alert("Failed to save Sales information " + e.message);
      }
    } else {
                              ...
                              <div className="col-lg-1">
                                <div className="input-box">
                                  ...
                                  <div className="form-group">
                                    ...
                                    <input className="form-control" value="981" type="text" {...register("Sales_id")} />
                                  </div>
                                  {errors.Sales_id && <p className="text-danger">{errors.Sales_id.message}</p>}
                                </div>
                              </div>
                              ...
                              <div className="col-lg-2">
                                <div className="input-box">
                                  ...
                                  <div className="form-group">
                                    ...
                                    <input className="form-control" type="date" {...register("Sales_date")} />
                                  </div>
                                  {errors.Sales_date && <p className="text-danger">{errors.Sales_date.message}</p>}
                                </div>
                              </div>
